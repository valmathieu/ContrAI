...
- (core) **BREAKING:** `PlayObservation`'s trick records are now sealed: `completed_tricks` / `current_trick` carry the new `ObservedPlay` `(position, card)` pairs instead of live-player `Play` records, and `current_winner` reports the winning `Position` — closing the `play.player.hand` path through which a card-play strategy could technically read another seat's cards. The expert AI's card tracking keys its void map by seat `Position` accordingly; the auction-side values (`contract`, `bids`) still name the players who bid.
