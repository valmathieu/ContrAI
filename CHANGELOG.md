...
- (engine) The card-point rule is implemented once. Round scoring, the in-game "Round pts" line and the recap's Outcome table all read `PlayState.card_points_by_side` / `trick_counts_by_side` instead of each re-summing the round's trick piles — the two screens were carrying their own copy of the trump-aware sum, and the view's copy is what the player actually reads. Who took the last trick now comes from the play state's per-trick winners for the same reason. Every number is unchanged; the running "Round pts" line now refreshes as soon as a trick's fourth card lands, rather than one frame later at the trick-won pause, because the play state credits the trick the moment it completes where the old mirror lagged a frame behind.
- (engine) **BREAKING:** The view's trick hooks — `request_card_action`, `on_card_played`, `on_trick_complete` — receive a sequence of core `Play` records instead of a `Trick` object, and `RichView.last_completed_trick` holds the same. A `Play` is a `(player, card)` `NamedTuple`, so it unpacks exactly like what `Trick.get_plays()` returned; what changes is that an in-progress trick and a completed `TrickRecord` now reach the screens as one type. Which of the eight tricks is on the table is resolved once per frame and handed to the panels that label it, instead of each panel counting a list.
...
- (engine) **BREAKING:** `Round`'s trick mirror is gone — `tricks`, `current_trick`, `team_tricks` and `last_trick_winner`, along with `play_all_tricks`' return value (which `Game` discarded). All four were kept in lock-step with the authoritative `PlayState` so the view could go on reading classic engine objects, but once scoring and the screens moved onto the state they were duplication with no remaining authority: the winner was recomputed on the mirror while the state already held it, and `tricks` was only ever read through `len()` — a counter wearing a `List[Trick]`. The played-out round now lives entirely on `Round.play_state`, and a mirror that could lag the state by a frame no longer exists to lag. Core's `Trick` class is untouched; the engine simply stops importing it.
...
- (engine) The in-game "Last trick" panel is numbered with the trick it actually shows. At the trick-won pause it read one too high — it was labelled from the count of completed tricks, which by then already included the trick sitting in the *Current* panel next to it, while the echo itself still showed the one before.
