...
| ------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Landing**   | program start, `n` from end-game        | The table-setup screen. A four-row summary of what the next deal will be played at — where the ruleset came from and how many knobs it moves off classic, the target score, the trump choices, the live round score — over a dispatcher: `[Enter]` deals, `[p]` opens the preset picker, `[f]` loads a setup file, `[l]` toggles the live round score. Each sub-screen returns a `TableSetup`, which becomes the one the summary redraws, so the panel always describes the table `[Enter]` would actually seat. Hardcoded seating today: South = `HumanPlayer`, N/E/W = `AiPlayer` (expert — the default).                                                         |
