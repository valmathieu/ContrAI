...
| ------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Landing**   | program start, `n` from end-game        | The table-setup screen. A four-row summary of what the next deal will be played at — where the ruleset came from and how many knobs it moves off classic, the target score, the trump choices, the live round score — over a dispatcher: `[Enter]` deals, `[p]` opens the preset picker, `[f]` loads a setup file, `[k]` the per-knob editor, `[l]` toggles the live round score. Each sub-screen returns a `TableSetup`, which becomes the one the summary redraws, so the panel always describes the table `[Enter]` would actually seat. The **knob editor** walks the six §9 subsections (`[n]` / `[b]`), listing each knob under its own `RuleConfig` / TOML field name with its value right-aligned and gold when it differs from classic — the grid is its own diff against the defaults. A number *cycles* that knob rather than prompting for a value, which is what lets one key reach every setting whether it is a bool, one of three enum members or a rung of the target ladder; each wraps. The two combinations §9 calls impossible are refused by core itself and rendered inline, leaving the ruleset as it was. Hardcoded seating today: South = `HumanPlayer`, N/E/W = `AiPlayer` (expert — the default).                                                         |
...
The pure helpers (bid parser, card parser, hand sorter, current-winner, constraint hint, illegal-bid reason, delay resolver, compact `Bid` label) are module-level functions in their respective modules (`parsing`, `state_helpers`, `bidding_rules`, `formatting`), and the per-screen `Panel` / `Table` builders are pure functions under `screens/`. The test suite mirrors that split — `tests/test_view/test_{formatting,parsing,bidding_rules,state_helpers,layout,setup,recap,endgame}.py` test the extracted modules, while `test_rich_view.py` keeps the stateful `RichView` behaviour (hooks, input loops, in-game frame). The setup screen's four input loops are driven there the way the bid loop already is: `view.console.input` is stubbed with an iterator, so a scripted keystroke sequence runs headless. The shared `four_players` fixture lives in `tests/test_view/conftest.py`. The deepest `Panel` / `Table` layouts not asserted on are validated end-to-end by smoke-running `uv run contrai`.
...
`cli.py`'s `main()` is the controller role today — there is no separate controller class. It builds a `RichView` and hands the landing screen the `TableSetup` the flags resolved to; `show_landing(setup)` returns the setup the player chose to deal under. From there two nested loops run: an outer **new-game / rematch** loop and an inner **round** loop that runs while `game.check_game_over().game_over` is false. Each outer tick `_build_game(rules=setup.rules)`s a fresh `Game` — South `HumanPlayer`, N/E/W expert `AiPlayer` — so the model owns every rule from construction on, and `check_game_over()` takes no argument: it reads `game.rules.target_score`. `view.attach(game, target_score=game.rules.target_score)` hands the view that same number rather than a parallel copy, which is what keeps the score panel, the recap and the loop from drifting apart. The interface aids take the other route: they never reach the model, so `main()` re-points `view.aids` each time the landing screen returns. Each round tick calls `game.manage_round(view=view)` (detailed in [Round lifecycle](#round-lifecycle)), then `view.on_round_complete` to feed the scoreboard and `view.show_round_recap(..., is_final=…)` to block on the between-round panel. When the game is over, `view.show_end_game(status)` returns `'q'` (break), `'n'` (re-open the landing screen on the setup in play), or `'r'` (rematch under the same setup). A top-level `try/except (KeyboardInterrupt, EOFError)` turns Ctrl-C / Ctrl-D into a graceful "Goodbye." rather than a traceback.
...
`Game.check_game_over(target)` ends the game when one side **strictly leads** at or above the target; a tie there is sudden death and keeps it running. One §9.6 knob qualifies the "at or above": `win_on_belote_points_alone` (on by default). Switched off, a side carried past the target by the belote of the very round that crossed it has not won yet — the win waits until points from play confirm it.

The gate looks at *that round's* belote only, which is why `Game` keeps a `last_round_belote` snapshot rather than a running belote total: by the time a later round is played, belote banked earlier is ordinary score and is not discounted. All of the crossing round's belote is discounted together, up to the four an all-trump round can award. The snapshot is refreshed on **both** exits of `manage_round` — a redeal awards none, so leaving a stale credit in place would discount points the crossing round never paid and deny a legitimate win. The gate never breaks a tie (sudden death is decided before it is consulted) and tolerates an unscored game, since `cli.py` checks before the first round is dealt.
...
`rounding` (§7.4, `exact` by default) is applied last, to each side's finished mark: `nearest_10` and `nearest_5` both round halves **up**, so a raw 85 marks 90. §7.4 calls out the consequence rather than treating it as an error — an 85-77 split marks 90-80, exceptionally 170 in total.

Two things keep it out of the way of everything else. It runs *after* belote is added, which is safe because the flat components (160, the 250 / 500 substitutes, the contract value) and the belote's 20 are all multiples of ten — in practice only a shared pile ever moves, and rounding before or after the belote is the same number. And it moves the marks only: whether the contract was made is judged on exact points, upstream, so no rounding rule can rescue a contract that fell short. `RoundScore.marks` and `card_points` stay unrounded, so the recap still shows what was really captured.
...
The `RuleConfig` a `Game` is built under is threaded down to `Round` and `PlayState` and read wherever the rule it names actually lands — no knob is copied, cached or re-derived. The flow and card-play switches are these.

**Turn direction** (§9.1) is *one* setting governing four things at once: who is dealt to, who speaks first in the auction, who leads trick 1, and who deals next. `Game.next_dealer` and `Game.set_players_order` both walk `Position.next_in(rules.turn_direction)` rather than doing `(index ± 1) % 4` against the roster, which is what keeps the four from ever disagreeing. Nothing downstream needs a direction of its own: `PlayState` steps `+1` through the `players` tuple it is handed, and that tuple *is* `players_order`, already running the table's way. Partners still land at indices 0 and 2 either way round, because seats alternate sides whichever direction play runs — which is exactly the invariant `PlayState`'s stepping relies on. `Position.partner`, `.opponents` and `.team_side` therefore take no direction parameter at all.

**The target score** (§9.1) is a table rule too, and no longer a view constant. `theme.TARGET_OPTIONS` is now one row per value of core's `TARGET_SCORES` — the ladder `RuleConfig` validates against — so the radio and the ruleset can never disagree about what is selectable; the label and the time estimate beside each value are the only presentation left in that table, and `DEFAULT_TARGET` derives from `RuleConfig().target_score`, which is **2000**, not the 1500 the view used to assert. `Game.check_game_over()` reads the number off `rules` and takes no argument at all, so there is no way to ask the question against a different target than the one the game is being played to.

**The Solo Slam declarer's lead** (§9.5, off by default) moves the *first* lead and nothing else. The opening lead normally belongs to the seat after the dealer whichever team took the contract; switched on, a Solo Slam declarer opens trick 1 instead. `Round._play_seating()` applies it by **rotating** the play order onto the declarer rather than rebuilding it, and that distinction is the whole design: `PlayState` steps `+1` through the tuple it is handed, so a rotation preserves the cyclic order. Play therefore still runs in the table's direction, later tricks are still led by whoever won the previous one, and the next dealer is still the seat after this one. The round's own `players_order` — the order the deal and the auction speak — is left untouched; only what the play state is *seeded* with rotates. Both seeding sites (`play_all_tricks` and `play_trick`'s lazy guard) go through the one helper, so they cannot disagree.
