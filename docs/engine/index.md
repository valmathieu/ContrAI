...
| ------------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Landing**   | program start, `n` from end-game        | Target-score picker over core's own `TARGET_SCORES` ladder (500 / 1000 / 1500 / 2000 / 3000 / 4000 / 5000), defaulting to §9.1's 2000 and seeded from the resolved ruleset's target. Hardcoded seating today: South = `HumanPlayer`, N/E/W = `AiPlayer` (expert — the default).                                                         |
...
`cli.py`'s `main()` is the controller role today — there is no separate controller class. It builds a `RichView`, asks for a target score on the landing screen (pre-selecting the resolved ruleset's own target), then runs two nested loops: an outer **new-game / rematch** loop and an inner **round** loop that runs while `game.check_game_over().game_over` is false. Each outer tick folds the landing pick onto the ruleset with `dataclasses.replace(rules, target_score=target)` *before* `_build_game()`s a fresh `Game` — South `HumanPlayer`, N/E/W expert `AiPlayer` — so the model owns the target from construction on and `check_game_over()` takes no argument: it reads `game.rules.target_score`. `view.attach(game, target_score=game.rules.target_score)` hands the view that same number rather than a parallel copy, which is what keeps the score panel, the recap and the loop from drifting apart. Each round tick calls `game.manage_round(view=view)` (detailed in [Round lifecycle](#round-lifecycle)), then `view.on_round_complete` to feed the scoreboard and `view.show_round_recap(..., is_final=…)` to block on the between-round panel. When the game is over, `view.show_end_game(status)` returns `'q'` (break), `'n'` (re-prompt the landing target), or `'r'` (rematch on the same target — the fold is idempotent, so replaying it changes nothing). A top-level `try/except (KeyboardInterrupt, EOFError)` turns Ctrl-C / Ctrl-D into a graceful "Goodbye." rather than a traceback.
