...
class TestAiPlayerStrategyInjection:
    """Test that AiPlayer injects and delegates to its bidding strategy."""

    def test_default_bidding_strategy_is_rule_based(self):
        """An AiPlayer built with defaults gets the rule-based bidder."""
        player = AiPlayer("Bot", "South")
        assert isinstance(player.bidding, RuleBasedBiddingStrategy)
        # The strategy reads player state live through its back-reference.
    ...
    def test_custom_injected_bidding_factory_is_used(self):
        """A bidding factory passed at construction replaces the default."""

        class StubBidding(RuleBasedBiddingStrategy):
            pass

        player = AiPlayer("Bot", "South", bidding=StubBidding)
